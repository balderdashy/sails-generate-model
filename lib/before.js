...
module.exports = function(scope, cb) {
	...
	if ( ! scope.rootPath ) {
		return cb.log.info('Usage: sails generate model <modelname> [attribute|attribute:type ...]');
	}

	// Check that we're in a valid sails project
	if (!fs.existsSync(path.resolve(scope.rootPath, 'api/models'))) {
		return cb.error('Sorry, this command can only be used in the root directory of a Sails project.');
	}
